...
import {
  ...
  selectorFind,
  Item,
} from "../Redux/slices/cardSlice.ts";

export default function PizzaCartCard(props: any) {
  const item = props.item as Item;
  const dispatch = useDispatch();
  const cartItem = useSelector(selectorFind(item));
  return (
      ...
      <div className="cart__item-count">
        <button
          ...
          className={clsx(
            ...
            { "cart__item-count-minus--disabled": cartItem?.count === 1 }
          )}
        >
          <svg
            width="10"
